/*  ---------------- FUNCTIONS ----------------- */
stock GiayPhepSung(playerid)
{
    if(GetPlayerWeapon(playerid))
    {
        if(PlayerInfo[playerid][pAdmin] == 0 && GetPlayerWeapon(playerid) != 9 && PlayerInfo[playerid][pGiayPhep] == 0 && !IsACop(playerid))
        {
            SetPlayerArmedWeapon(playerid, 0);
        	SendClientMessageEx(playerid, COLOR_GRAD2, "Ban hien chua dang ky giay phep su dung vu khi !");
			return 1;
		}
	    else
	    {
...
NationSel_HandleNationSelection(playerid)
{
	...
	if(Keys & KEY_FIRE)
	{
		...
		SendClientMessage(playerid, COLOR_YELLOW, "CHAO MUNG DEN MAY CHU WOLRP.NET");
		SendClientMessage(playerid, COLOR_YELLOW, "Su dung /timvieclam /timduong de kiem viec lam");
		SendClientMessage(playerid, COLOR_YELLOW, "Hay gioi thieu ban be qua choi , Tao san choi lanh manh");
		...
		if(NATION_SAN_ANDREAS == PlayerNationSelection[playerid])
		{
			switch(random(2))
			{
				case 0:
				{
					SetPlayerPos(playerid, -2633.6917, 600.2280, 11.7832);
...
ShowCasinoGamesMenu(playerid, dialogid)
{
	switch(dialogid)
	{
		...
		{
			return ShowPlayerDialog(playerid, DIALOG_CGAMESCREDITS, DIALOG_STYLE_MSGBOX, "{FFFFFF}Casino Games - (Creadits)", "{FFFFFF}Chinh sua boi: SA-MP VN Blog", "Quay Lai", "");
		}
...
public InitiateGamemode()
{
    ...
    // Bai toan
    //LoadGame();
	//SetTimer("LoadGame",E_TIME*999999999,true);
	SetTimer("TimeSV", 1000, true);
...
public sobeitCheck(playerid)
{
    ...
    if(IsPlayerFrozen[playerid] == 1)
	{
        ...
        {
            SendClientMessageEx(playerid, COLOR_RED, "Server phat hien ban co dau hieu Hack/Cheat vui long xoa ngay neu khong se bi khoa tai khoan!");
...
forward MayDau1(playerid);
public MayDau1(playerid)
{
    SetPVarInt(playerid, "MayDauTime1", GetPVarInt(playerid, "MayDauTime1")-1);

    new labelmay1[1280];
	format(labelmay1, sizeof(labelmay1), "{009933}MAY DAU (( 1 )){ffffff}\nTinh Trang : %d%\nNeu tinh trang may hu hong qua nang , ban buoc phai den tien cho nha may'\n{00ffff}Hien dang duoc sua chua{ffffff}.", TinhTrangMay1);
    UpdateDynamic3DTextLabelText(MayDau_[0], COLOR_GREY, labelmay1);
    TinhTrangMay1 += 1;
    ApplyAnimation(playerid,"SHOP", "SHP_Rob_GiveCash",4.0,0,0,0,0,0,1);
    if(TinhTrangMay1 < 100) SetTimerEx("MayDau1", 1000, 0, "d", playerid);


    if(TinhTrangMay1 == 100)
    {
        DeletePVar(playerid, "MayDauTime1");
        DeletePVar(playerid, "MayDau1");
		...
		format(labelmay1, sizeof(labelmay1), "{009933}MAY DAU (( 1 )){ffffff}\nTinh Trang : %d%\n{ff00ff}Hoat dong tot{ffffff}.", TinhTrangMay1);
	    UpdateDynamic3DTextLabelText(MayDau_[0], COLOR_GREY, labelmay1);

        DangSuaMayDau[playerid] = 0;

        new money = 5000+random(2000);
        new string[1280];
    	format(string, sizeof(string), "Ban nhan duoc %d , tu viec sua chua thanh cong May Dau Khi (( 1 ))", money);
        SendClientMessageEx(playerid, COLOR_GREEN, string);
        SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da sua chua thanh cong May Dau Khi (( 1 ))");
        return 1;
    }
    return 1;
}

forward MayDau2(playerid);
public MayDau2(playerid)
{
    SetPVarInt(playerid, "MayDauTime2", GetPVarInt(playerid, "MayDauTime2")-1);

    new labelmay2[1280];
	format(labelmay2, sizeof(labelmay2), "{009933}MAY DAU (( 2 )){ffffff}\nTinh Trang : %d%\nNeu tinh trang may hu hong qua nang , ban buoc phai den tien cho nha may'\n{00ffff}Hien dang duoc sua chua{ffffff}.", TinhTrangMay2);
    UpdateDynamic3DTextLabelText(MayDau_[1], COLOR_GREY, labelmay2);
    TinhTrangMay2 += 1;
    ApplyAnimation(playerid,"SHOP", "SHP_Rob_GiveCash",4.0,0,0,0,0,0,1);
    if(TinhTrangMay2 < 100) SetTimerEx("MayDau2", 1000, 0, "d", playerid);


    if(TinhTrangMay2 == 100)
    {
        DeletePVar(playerid, "MayDauTime2");
        DeletePVar(playerid, "MayDau2");
		...
		format(labelmay2, sizeof(labelmay2), "{009933}MAY DAU (( 2 )){ffffff}\nTinh Trang : %d%\n{ff00ff}Hoat dong tot{ffffff}.", TinhTrangMay2);
	    UpdateDynamic3DTextLabelText(MayDau_[1], COLOR_GREY, labelmay2);
        ...
        new string[1280];
        new money = 5000+random(2000);
    	format(string, sizeof(string), "Ban nhan duoc %d , tu viec sua chua thanh cong May Dau Khi (( 2 ))", money);
        SendClientMessageEx(playerid, COLOR_GREEN, string);

        SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da sua chua thanh cong May Dau Khi (( 2 ))");
        return 1;
    }
    return 1;
}

forward MayDau3(playerid);
public MayDau3(playerid)
{
    SetPVarInt(playerid, "MayDauTime3", GetPVarInt(playerid, "MayDauTime3")-1);

    new labelmay3[1280];
	format(labelmay3, sizeof(labelmay3), "{009933}MAY DAU (( 3 )){ffffff}\nTinh Trang : %d%\nNeu tinh trang may hu hong qua nang , ban buoc phai den tien cho nha may'\n{00ffff}Hien dang duoc sua chua{ffffff}.", TinhTrangMay3);
    UpdateDynamic3DTextLabelText(MayDau_[2], COLOR_GREY, labelmay3);
    TinhTrangMay3 += 1;
    ApplyAnimation(playerid,"SHOP", "SHP_Rob_GiveCash",4.0,0,0,0,0,0,1);
    if(TinhTrangMay3 < 100) SetTimerEx("MayDau3", 1000, 0, "d", playerid);


    if(TinhTrangMay3 == 100)
    {
        DeletePVar(playerid, "MayDauTime3");
        DeletePVar(playerid, "MayDau3");
		...
		format(labelmay3, sizeof(labelmay3), "{009933}MAY DAU (( 3 )){ffffff}\nTinh Trang : %d%\n{ff00ff}Hoat dong tot{ffffff}.", TinhTrangMay3);
	    UpdateDynamic3DTextLabelText(MayDau_[2], COLOR_GREY, labelmay3);
        ...
        new string[1280];
        new money = 5000+random(2000);
    	format(string, sizeof(string), "Ban nhan duoc %d , tu viec sua chua thanh cong May Dau Khi (( 3 ))", money);
        SendClientMessageEx(playerid, COLOR_GREEN, string);

        SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da sua chua thanh cong May Dau Khi (( 3 ))");
        return 1;
    }
    return 1;
}

forward MayDau4(playerid);
public MayDau4(playerid)
{
    SetPVarInt(playerid, "MayDauTime4", GetPVarInt(playerid, "MayDauTime4")-1);

    new labelmay4[1280];
	format(labelmay4, sizeof(labelmay4), "{009933}MAY DAU (( 4 )){ffffff}\nTinh Trang : %d%\nNeu tinh trang may hu hong qua nang , ban buoc phai den tien cho nha may'\n{00ffff}Hien dang duoc sua chua{ffffff}.", TinhTrangMay4);
    UpdateDynamic3DTextLabelText(MayDau_[3], COLOR_GREY, labelmay4);
    TinhTrangMay4 += 1;
    ApplyAnimation(playerid,"SHOP", "SHP_Rob_GiveCash",4.0,0,0,0,0,0,1);
    if(TinhTrangMay4 < 100) SetTimerEx("MayDau4", 1000, 0, "d", playerid);


    if(TinhTrangMay4 == 100)
    {
        DeletePVar(playerid, "MayDauTime4");
        DeletePVar(playerid, "MayDau4");
		...
		format(labelmay4, sizeof(labelmay4), "{009933}MAY DAU (( 4 )){ffffff}\nTinh Trang : %d%\n{ff00ff}Hoat dong tot{ffffff}.", TinhTrangMay4);
	    UpdateDynamic3DTextLabelText(MayDau_[3], COLOR_GREY, labelmay4);
        ...
        new string[1280];
        new money = 5000+random(2000);
    	format(string, sizeof(string), "Ban nhan duoc %d , tu viec sua chua thanh cong May Dau Khi (( 4 ))", money);
        SendClientMessageEx(playerid, COLOR_GREEN, string);

        SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da sua chua thanh cong May Dau Khi (( 4 ))");
        return 1;
    }
    return 1;
}

forward MayDau5(playerid);
public MayDau5(playerid)
{
    SetPVarInt(playerid, "MayDauTime5", GetPVarInt(playerid, "MayDauTime5")-1);

    new labelmay5[1280];
	format(labelmay5, sizeof(labelmay5), "{009933}MAY DAU (( 5 )){ffffff}\nTinh Trang : %d%\nNeu tinh trang may hu hong qua nang , ban buoc phai den tien cho nha may'\n{00ffff}Hien dang duoc sua chua{ffffff}.", TinhTrangMay5);
    UpdateDynamic3DTextLabelText(MayDau_[4], COLOR_GREY, labelmay5);
    TinhTrangMay5 += 1;
    ApplyAnimation(playerid,"SHOP", "SHP_Rob_GiveCash",4.0,0,0,0,0,0,1);
    if(TinhTrangMay5 < 100) SetTimerEx("MayDau5", 1000, 0, "d", playerid);


    if(TinhTrangMay5 == 100)
    {
        DeletePVar(playerid, "MayDauTime5");
        DeletePVar(playerid, "MayDau5");
		...
		format(labelmay5, sizeof(labelmay5), "{009933}MAY DAU (( 5 )){ffffff}\nTinh Trang : %d%\n{ff00ff}Hoat dong tot{ffffff}.", TinhTrangMay5);
	    UpdateDynamic3DTextLabelText(MayDau_[4], COLOR_GREY, labelmay5);
        ...
        new string[1280];
        new money = 5000+random(2000);
    	format(string, sizeof(string), "Ban nhan duoc %d , tu viec sua chua thanh cong May Dau Khi (( 5 ))", money);
        SendClientMessageEx(playerid, COLOR_GREEN, string);

        SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da sua chua thanh cong May Dau Khi (( 5 ))");
        return 1;
    }
    return 1;
}

forward MayDau6(playerid);
public MayDau6(playerid)
{
    SetPVarInt(playerid, "MayDauTime6", GetPVarInt(playerid, "MayDauTime6")-1);

    new labelmay6[1280];
	format(labelmay6, sizeof(labelmay6), "{009933}MAY DAU (( 6 )){ffffff}\nTinh Trang : %d%\nNeu tinh trang may hu hong qua nang , ban buoc phai den tien cho nha may'\n{00ffff}Hien dang duoc sua chua{ffffff}.", TinhTrangMay6);
    UpdateDynamic3DTextLabelText(MayDau_[5], COLOR_GREY, labelmay6);
    TinhTrangMay6 += 1;
    ApplyAnimation(playerid,"SHOP", "SHP_Rob_GiveCash",4.0,0,0,0,0,0,1);
    if(TinhTrangMay6 < 100) SetTimerEx("MayDau6", 1000, 0, "d", playerid);


    if(TinhTrangMay6 == 100)
    {
        DeletePVar(playerid, "MayDauTime6");
        DeletePVar(playerid, "MayDau6");
		...
		format(labelmay6, sizeof(labelmay6), "{009933}MAY DAU (( 6 )){ffffff}\nTinh Trang : %d%\n{ff00ff}Hoat dong tot{ffffff}.", TinhTrangMay6);
	    UpdateDynamic3DTextLabelText(MayDau_[5], COLOR_GREY, labelmay6);
        ...
        new string[1280];
        new money = 5000+random(2000);
    	format(string, sizeof(string), "Ban nhan duoc %d , tu viec sua chua thanh cong May Dau Khi (( 6 ))", money);
        SendClientMessageEx(playerid, COLOR_GREEN, string);

        SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da sua chua thanh cong May Dau Khi (( 6 ))");
        return 1;
    }
    return 1;
}

forward MayDau7(playerid);
public MayDau7(playerid)
{
    SetPVarInt(playerid, "MayDauTime7", GetPVarInt(playerid, "MayDauTime7")-1);

    new labelmay7[1280];
	format(labelmay7, sizeof(labelmay7), "{009933}MAY DAU (( 7 )){ffffff}\nTinh Trang : %d%\nNeu tinh trang may hu hong qua nang , ban buoc phai den tien cho nha may'\n{00ffff}Hien dang duoc sua chua{ffffff}.", TinhTrangMay7);
    UpdateDynamic3DTextLabelText(MayDau_[6], COLOR_GREY, labelmay7);
    TinhTrangMay7 += 1;
    ApplyAnimation(playerid,"SHOP", "SHP_Rob_GiveCash",4.0,0,0,0,0,0,1);
    if(TinhTrangMay7 < 100) SetTimerEx("MayDau7", 1000, 0, "d", playerid);


    if(TinhTrangMay7 == 100)
    {
        DeletePVar(playerid, "MayDauTime7");
        DeletePVar(playerid, "MayDau7");
		...
		format(labelmay7, sizeof(labelmay7), "{009933}MAY DAU (( 7 )){ffffff}\nTinh Trang : %d%\n{ff00ff}Hoat dong tot{ffffff}.", TinhTrangMay7);
	    UpdateDynamic3DTextLabelText(MayDau_[6], COLOR_GREY, labelmay7);
        ...
        new string[1280];
        new money = 5000+random(2000);
    	format(string, sizeof(string), "Ban nhan duoc %d , tu viec sua chua thanh cong May Dau Khi (( 7 ))", money);
        SendClientMessageEx(playerid, COLOR_GREEN, string);

        SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da sua chua thanh cong May Dau Khi (( 7 ))");
        return 1;
    }
    return 1;
}

forward MayDau8(playerid);
public MayDau8(playerid)
{
    SetPVarInt(playerid, "MayDauTime8", GetPVarInt(playerid, "MayDauTime8")-1);

    new labelmay8[1280];
	format(labelmay8, sizeof(labelmay8), "{009933}MAY DAU (( 8 )){ffffff}\nTinh Trang : %d%\nNeu tinh trang may hu hong qua nang , ban buoc phai den tien cho nha may'\n{00ffff}Hien dang duoc sua chua{ffffff}.", TinhTrangMay8);
    UpdateDynamic3DTextLabelText(MayDau_[7], COLOR_GREY, labelmay8);
    TinhTrangMay8 += 1;
    ApplyAnimation(playerid,"SHOP", "SHP_Rob_GiveCash",4.0,0,0,0,0,0,1);
    if(TinhTrangMay8 < 100) SetTimerEx("MayDau8", 1000, 0, "d", playerid);


    if(TinhTrangMay8 == 100)
    {
        DeletePVar(playerid, "MayDauTime8");
        DeletePVar(playerid, "MayDau8");
		...
		format(labelmay8, sizeof(labelmay8), "{009933}MAY DAU (( 8 )){ffffff}\nTinh Trang : %d%\n{ff00ff}Hoat dong tot{ffffff}.", TinhTrangMay8);
	    UpdateDynamic3DTextLabelText(MayDau_[7], COLOR_GREY, labelmay8);
        ...
        new string[1280];
        new money = 5000+random(2000);
    	format(string, sizeof(string), "Ban nhan duoc %d , tu viec sua chua thanh cong May Dau Khi (( 8 ))", money);
        SendClientMessageEx(playerid, COLOR_GREEN, string);

        SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da sua chua thanh cong May Dau Khi (( 8 ))");
        return 1;
    }
    return 1;
}

forward MayDau9(playerid);
public MayDau9(playerid)
{
    SetPVarInt(playerid, "MayDauTime9", GetPVarInt(playerid, "MayDauTime9")-1);

    new labelmay9[1280];
	format(labelmay9, sizeof(labelmay9), "{009933}MAY DAU (( 9 )){ffffff}\nTinh Trang : %d%\nNeu tinh trang may hu hong qua nang , ban buoc phai den tien cho nha may'\n{00ffff}Hien dang duoc sua chua{ffffff}.", TinhTrangMay9);
    UpdateDynamic3DTextLabelText(MayDau_[8], COLOR_GREY, labelmay9);
    TinhTrangMay9 += 1;
    ApplyAnimation(playerid,"SHOP", "SHP_Rob_GiveCash",4.0,0,0,0,0,0,1);
    if(TinhTrangMay9 < 100) SetTimerEx("MayDau9", 1000, 0, "d", playerid);


    if(TinhTrangMay9 == 100)
    {
        DeletePVar(playerid, "MayDauTime9");
        DeletePVar(playerid, "MayDau9");
		...
		format(labelmay9, sizeof(labelmay9), "{009933}MAY DAU (( 9 )){ffffff}\nTinh Trang : %d%\n{ff00ff}Hoat dong tot{ffffff}.", TinhTrangMay9);
	    UpdateDynamic3DTextLabelText(MayDau_[8], COLOR_GREY, labelmay9);
        ...
        new string[1280];
        new money = 5000+random(2000);
    	format(string, sizeof(string), "Ban nhan duoc %d , tu viec sua chua thanh cong May Dau Khi (( 9 ))", money);
        SendClientMessageEx(playerid, COLOR_GREEN, string);

        SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da sua chua thanh cong May Dau Khi (( 9 ))");
        return 1;
    }
    return 1;
}

forward MayDau10(playerid);
public MayDau10(playerid)
{
    SetPVarInt(playerid, "MayDauTime10", GetPVarInt(playerid, "MayDauTime10")-1);

    new labelmay10[1280];
	format(labelmay10, sizeof(labelmay10), "{009933}MAY DAU (( 10 )){ffffff}\nTinh Trang : %d%\nNeu tinh trang may hu hong qua nang , ban buoc phai den tien cho nha may'\n{00ffff}Hien dang duoc sua chua{ffffff}.", TinhTrangMay10);
    UpdateDynamic3DTextLabelText(MayDau_[9], COLOR_GREY, labelmay10);
    TinhTrangMay10 += 1;
    ApplyAnimation(playerid,"SHOP", "SHP_Rob_GiveCash",4.0,0,0,0,0,0,1);
    if(TinhTrangMay10 < 100) SetTimerEx("MayDau10", 1000, 0, "d", playerid);


    if(TinhTrangMay10 == 100)
    {
        DeletePVar(playerid, "MayDauTime10");
        DeletePVar(playerid, "MayDau10");
		...
		format(labelmay10, sizeof(labelmay10), "{009933}MAY DAU (( 10 )){ffffff}\nTinh Trang : %d%\n{ff00ff}Hoat dong tot{ffffff}.", TinhTrangMay10);
	    UpdateDynamic3DTextLabelText(MayDau_[9], COLOR_GREY, labelmay10);
        ...
        new string[1280];
        new money = 5000+random(2000);
    	format(string, sizeof(string), "Ban nhan duoc %d , tu viec sua chua thanh cong May Dau Khi (( 10 ))", money);
        SendClientMessageEx(playerid, COLOR_GREEN, string);

        SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da sua chua thanh cong May Dau Khi (( 10 ))");
        return 1;
    }
    return 1;
}
...
public Maintenance()
{
	...
	SendRconCommand("hostname Bao tri may chu");
	foreach(new i: Player)
	{
		if(gPlayerLogged{i}) {
	...
	if(GetPVarInt(playerid, "HelpTime") > 0)
 	{
    	...
    	if(GetPVarInt(playerid, "HelpTime") == 0)
     	{
      		SendClientMessageEx(playerid, COLOR_GREY, "Yeu cau tro giup cua ban da het han. Neu day la mot loi hay tham gia dien dan forum.gta-sa.vn de bao loi.");
...
forward CNChet(playerid);
public CNChet(playerid)
{
	SetPVarInt(playerid, "CNChetTime", GetPVarInt(playerid, "CNChetTime")-1);
	new string[128];
	format(string, sizeof(string), "~n~~n~~n~~n~~n~~n~~n~~n~~n~~w~%d giay", GetPVarInt(playerid, "CNChetTime"));
	GameTextForPlayer(playerid, string, 1100, 3);

	if(GetPVarInt(playerid, "CNChetTime") > 0) SetTimerEx("CNChet", 6000, 0, "d", playerid);

	if(GetPVarInt(playerid, "CNChetTime") <= 0)
	{
        ResetPlayerWeaponsEx(playerid);
        SpawnPlayer(playerid);
        DeletePVar(playerid, "CNChetTime");
	}
}
...
stock SetPlayerJoinCamera(playerid)
{
	...
	switch(randcamera)
	{
		case 1: // Benh Vien SF
		{
			SetPlayerVirtualWorld(playerid, 0);
			SetPlayerInterior(playerid, 0);
			InterpolateCameraPos(playerid, 1403.205322, -1406.083251, 15.045023, 1246.879638, -1386.249023, 14.675035, 10000);
			InterpolateCameraLookAt(playerid, 1398.215698, -1405.760986, 15.041118, 1242.833496, -1383.312744, 14.589101, 100);
		}
		case 2: // City Hall To Pizza
		{
			SetPlayerVirtualWorld(playerid, 0);
			SetPlayerInterior(playerid, 0);
			InterpolateCameraPos(playerid, 1693.223999, -1811.264770, 16.571004, 2085.521728, -1815.373291, 14.766554, 10000);
			InterpolateCameraLookAt(playerid, 1698.201293, -1811.710449, 16.403068, 2090.041259, -1813.239135, 14.625947, 1000);
		}
		case 3: // Nga Tu TRucker
		{
			SetPlayerVirtualWorld(playerid, 0);
			SetPlayerInterior(playerid, 0);
			InterpolateCameraPos(playerid, 1858.330200, -1289.086181, 18.652730, 1828.427978, -1571.079345, 17.113517, 12000);
			InterpolateCameraLookAt(playerid, 1857.646972, -1294.032958, 18.402835, 1823.524902, -1571.979736, 16.727184, 1000);
		}
		case 4: // CamEdit_LSPD
		{
			SetPlayerVirtualWorld(playerid, 0);
			SetPlayerInterior(playerid, 0);
			InterpolateCameraPos(playerid, 1280.827636, -1750.103271, 14.218022, 1531.998535, -1690.542602, 16.597816, 10000);
			InterpolateCameraLookAt(playerid, 1285.665405, -1748.840942, 14.268802, 1535.828002, -1687.328857, 16.511882, 1000);
		}
		case 5: // Trucker
		{
			SetPlayerVirtualWorld(playerid, 0);
...
stock NhacNen(playerid)
{
	new randmusic = Random(1,4);
	switch(randmusic)
	{
		case 1: // Benh Vien SF
		{
			PlayAudioStreamForPlayer(playerid, "https://uploadir.com/u/uxf2sipz");
		}
		case 2: // City Hall To Pizza
		{
			PlayAudioStreamForPlayer(playerid, "https://uploadir.com/u/uxf2sipz");
...
stock ShowMainMenuDialog(playerid, frame)
{
	...
	switch(frame)
	{
		case 1:
		{
			...
			format(str1, sizeof(str), "TAI KHOAN {ff3300}%s{FFFFFF} DA DUOC DANG KY", GetPlayerNameEx(playerid));
		    format(str2, sizeof(str), "   + ID TAI KHOAN : %i .", PlayerInfo[playerid][pId]);
		    format(str3, sizeof(str), "   + IP : %s ", GetPlayerIpEx(playerid));
		    format(str4, sizeof(str), "NEU DAY LA TAI KHOAN CUA BAN , VUI LONG DANG NHAP MAT KHAU DE DANG NHAP VAO BEN TRONG :");
		    format(str, sizeof(str), "%s\n\n%s\n\n%s\n\n%s", str1,str2,str3,str4);
		    format(name, sizeof(name), "{00ff00}DANG NHAP{FFFFFF}");
			ShowPlayerDialog(playerid, MAINMENU, DIALOG_STYLE_PASSWORD, name, str, "Dong Y", "Huy Bo");
			}
			else
			{
			    SendClientMessage(playerid, COLOR_RED, "Ban bi kick , do dang nhap vao bang PC.");
				SetTimerEx("KickEx", 1000, 0, "i", playerid);
			}
		}
		case 2:
		{
		    ...
		    {
			new str[2460], str1[1280], str2[1280], str3[1280], str4[1280], name[1280];
			format(str1, sizeof(str), "TAI KHOAN {ff3300}%s{FFFFFF} CHUA DUOC DANG KY VUI LONG MAT KHAU DE DANG NHAP VAO BEN TRONG", GetPlayerNameEx(playerid));
		    format(str2, sizeof(str), "   + ID TAI KHOAN : %i .", PlayerInfo[playerid][pId]);
		    format(str3, sizeof(str), "   + IP : %s ", GetPlayerIpEx(playerid));
		    format(str4, sizeof(str), "HIEN TAI TAI KHOAN CUA BAN, CHUA DUOC DANG KY VUI LONG MAT KHAU DE DANG NHAP VAO BEN TRONG");
		    format(str, sizeof(str), "%s\n\n%s\n\n%s\n\n%s\n\n%s", str1,str2,str3,str4);
		    format(name, sizeof(name), "{ff3300}DANG KY{FFFFFF}");
...
stock GetJobName(job)
{
	...
	switch(job)
	{
        ...
        case 24: name = "Ngu Dan";
        case 25: name = "Giao Bao";
        case 26: name = "Giao Trai Cay";
        case 27: name = "Tho Moc";
        case 28: name = "Van Chuyen Xang";
        case 29: name = "Nong Dan";
        case 30: name = "Dao Da";
...
stock HospitalSpawn(playerid)
{
	if(GetPVarInt(playerid, "MedicBill") == 1 && PlayerInfo[playerid][pJailTime] == 0)
	{
		...
		SetPlayerPos(playerid, -2655.2520,630.8400,14.4531);
		SetPlayerFacingAngle(playerid, 176.1327);
...
stock IsAtFishPlace(playerid)
{
	if(IsPlayerConnected(playerid))
	{
		if(IsPlayerInRangeOfPoint(playerid,3.0,-1464.0975, 791.0641, 1.9908) || IsPlayerInRangeOfPoint(playerid,3.0,-1464.0001, 784.3825, 1.2716))
		{//Fishplace at the bigwheel
			return 1;
		}
		else if(IsPlayerInRangeOfPoint(playerid,3.0,-1463.2455, 777.9412, -0.4898) || IsPlayerInRangeOfPoint(playerid,3.0,-1464.5549, 771.2213, 2.6316))
		{//Fishplace at the bigwheel
			return 1;
		}
		else if(IsPlayerInRangeOfPoint(playerid,3.0,-1463.6718, 728.3496, 2.2017) || IsPlayerInRangeOfPoint(playerid,3.0,-1464.3716, 721.3877, 2.8352))
		{//Fishplace at the bigwheel
			return 1;
		}
		else if(IsPlayerInRangeOfPoint(playerid,3.0,-1463.7322, 714.9433, 2.6410) || IsPlayerInRangeOfPoint(playerid,3.0,-1463.3711, 708.4399, 2.4798))
		{//Fishplace at the bigwheel
			return 1;
		}
...
public SyncTime()
{
	...
	if ((tmphour > ghour) || (tmphour == 0 && ghour == 23))
	{
		...
		foreach(new i: Player)
		{
			if(PlayerInfo[i][pLevel] <= 5) SendClientMessageEx(i,COLOR_LIGHTBLUE,"Viec su dung hack/cleo tai may chu WOLRP.NET ban se bi xoa tai khoan vinh vien khoi he thong!");
...
stock IsANguDanCar(carid)
{
	for(new v = 0; v < sizeof(NguDanVeh); v++) {
	    if(carid == NguDanVeh[v]) return 1;
	}
	return 0;
}
...
stock IsAtThoMoc(playerid)
{
    if(IsPlayerConnected(playerid))
    {
        if(IsPlayerInRangeOfPoint(playerid, 5.0, -541.7692,-168.1313,78.4063) || IsPlayerInRangeOfPoint(playerid, 5.0, -536.4109,-155.5589,77.4629) || IsPlayerInRangeOfPoint(playerid, 5.0, -484.4834,-156.1147,76.3770) || IsPlayerInRangeOfPoint(playerid, 5.0, -478.4091,-125.4227,66.3274) || IsPlayerInRangeOfPoint(playerid, 5.0, -479.3493,-116.8348,64.2930) || IsPlayerInRangeOfPoint(playerid, 5.0, -485.1117,-107.5717,63.1004) || IsPlayerInRangeOfPoint(playerid, 5.0, -482.0093,-85.3962,60.7033) || IsPlayerInRangeOfPoint(playerid, 5.0, -499.7765,-85.8111,61.5695) || IsPlayerInRangeOfPoint(playerid, 5.0, -558.5072,-49.0749,64.1251) || IsPlayerInRangeOfPoint(playerid, 5.0, -564.2932,-62.4542,64.2629) || IsPlayerInRangeOfPoint(playerid, 5.0, -568.2906,-99.4289,64.6815) || IsPlayerInRangeOfPoint(playerid, 5.0, -569.1982,-108.8848,66.3840) || IsPlayerInRangeOfPoint(playerid, 5.0, -521.9217,-200.8204,78.4063))
        {
...
stock ShowInventory(playerid,targetid)
{
	if(IsPlayerConnected(targetid))
	{
		...
		PlayerInfo[targetid][pCanSa],
		PlayerInfo[targetid][pTKNganHang],
		pnumber,
...
stock DisplayOrders(playerid)
{
	...
	// xe truck // truck
		...
		TogglePlayerControllable(playerid, 1);
		DeletePVar(playerid, "IsFrozen"); */
		if(GetVehicleModel(GetPlayerVehicleID(playerid)) == 456 || GetVehicleModel(GetPlayerVehicleID(playerid)) == 456 || IsABoat(GetPlayerVehicleID(playerid)))
		{
...
public CanSa1(playerid)
{
    ...
    new string[1280];
    format(string, sizeof(string), "~n~~n~~n~~n~~n~~n~~n~~n~~n~~w~%d giay con lai", GetPVarInt(playerid, "CanSaTime1"));
    GameTextForPlayer(playerid, string, 1100, 3);
    ...
    if(GetPVarInt(playerid, "CanSaTime1") <= 0)
    {
        ...
        PlayerCS[pCayCanSa1] += 1;

        PlayerInfo[playerid][pCanSa] += 1;

        DangHaiCan[playerid] = 0;
        SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da hai thanh cong mot cay can sa (+1 Can Sa)");
        return 1;
    }
    return 1;
}

forward CanSa2(playerid);
public CanSa2(playerid)
{
    SetPVarInt(playerid, "CanSaTime2", GetPVarInt(playerid, "CanSaTime2")-1);

    new string[1280];
    format(string, sizeof(string), "~n~~n~~n~~n~~n~~n~~n~~n~~n~~w~%d giay con lai", GetPVarInt(playerid, "CanSaTime2"));
    GameTextForPlayer(playerid, string, 1100, 3);
    ApplyAnimation(playerid,"BOMBER","BOM_Plant",4.0,0,0,0,0,0,1);
    if(GetPVarInt(playerid, "CanSaTime2") > 0) SetTimerEx("CanSa2", 1000, 0, "d", playerid);


    if(GetPVarInt(playerid, "CanSaTime2") <= 0)
    {
        DeletePVar(playerid, "CanSaTime2");
        DeletePVar(playerid, "CanSa2");

        TogglePlayerControllable(playerid, 1);
        ClearAnimations(playerid);

        PlayerCS[pCayCanSa2] += 1;
...
forward CanSa3(playerid);
public CanSa3(playerid)
{
    SetPVarInt(playerid, "CanSaTime3", GetPVarInt(playerid, "CanSaTime3")-1);

    new string[1280];
    format(string, sizeof(string), "~n~~n~~n~~n~~n~~n~~n~~n~~n~~w~%d giay con lai", GetPVarInt(playerid, "CanSaTime3"));
    GameTextForPlayer(playerid, string, 1100, 3);
    ApplyAnimation(playerid,"BOMBER","BOM_Plant",4.0,0,0,0,0,0,1);
    if(GetPVarInt(playerid, "CanSaTime3") > 0) SetTimerEx("CanSa3", 1000, 0, "d", playerid);


    if(GetPVarInt(playerid, "CanSaTime3") <= 0)
    {
        DeletePVar(playerid, "CanSaTime3");
        DeletePVar(playerid, "CanSa3");

        TogglePlayerControllable(playerid, 1);
        ClearAnimations(playerid);

        PlayerCS[pCayCanSa3] += 1;
...
forward CanSa4(playerid);
public CanSa4(playerid)
{
    SetPVarInt(playerid, "CanSaTime4", GetPVarInt(playerid, "CanSaTime4")-1);

    new string[1280];
    format(string, sizeof(string), "~n~~n~~n~~n~~n~~n~~n~~n~~n~~w~%d giay con lai", GetPVarInt(playerid, "CanSaTime4"));
    GameTextForPlayer(playerid, string, 1100, 3);
    ApplyAnimation(playerid,"BOMBER","BOM_Plant",4.0,0,0,0,0,0,1);
    if(GetPVarInt(playerid, "CanSaTime4") > 0) SetTimerEx("CanSa4", 1000, 0, "d", playerid);


    if(GetPVarInt(playerid, "CanSaTime4") <= 0)
    {
        DeletePVar(playerid, "CanSaTime4");
        DeletePVar(playerid, "CanSa4");

        TogglePlayerControllable(playerid, 1);
        ClearAnimations(playerid);

        PlayerCS[pCayCanSa4] += 1;
...
forward CanSa5(playerid);
public CanSa5(playerid)
{
    SetPVarInt(playerid, "CanSaTime5", GetPVarInt(playerid, "CanSaTime5")-1);

    new string[1280];
    format(string, sizeof(string), "~n~~n~~n~~n~~n~~n~~n~~n~~n~~w~%d giay con lai", GetPVarInt(playerid, "CanSaTime5"));
    GameTextForPlayer(playerid, string, 1100, 3);
    ApplyAnimation(playerid,"BOMBER","BOM_Plant",4.0,0,0,0,0,0,1);
    if(GetPVarInt(playerid, "CanSaTime5") > 0) SetTimerEx("CanSa5", 1000, 0, "d", playerid);


    if(GetPVarInt(playerid, "CanSaTime5") <= 0)
    {
        DeletePVar(playerid, "CanSaTime5");
        DeletePVar(playerid, "CanSa5");

        TogglePlayerControllable(playerid, 1);
        ClearAnimations(playerid);

        PlayerCS[pCayCanSa5] += 1;
...
forward CanSa6(playerid);
public CanSa6(playerid)
{
    SetPVarInt(playerid, "CanSaTime6", GetPVarInt(playerid, "CanSaTime6")-1);

    new string[1280];
    format(string, sizeof(string), "~n~~n~~n~~n~~n~~n~~n~~n~~n~~w~%d giay con lai", GetPVarInt(playerid, "CanSaTime6"));
    GameTextForPlayer(playerid, string, 1100, 3);
    ApplyAnimation(playerid,"BOMBER","BOM_Plant",4.0,0,0,0,0,0,1);
    if(GetPVarInt(playerid, "CanSaTime6") > 0) SetTimerEx("CanSa6", 1000, 0, "d", playerid);


    if(GetPVarInt(playerid, "CanSaTime6") <= 0)
    {
        DeletePVar(playerid, "CanSaTime6");
        DeletePVar(playerid, "CanSa6");

        TogglePlayerControllable(playerid, 1);
        ClearAnimations(playerid);

        PlayerCS[pCayCanSa6] += 1;
...
public FinishRobbery()
{
	...
	DestroyDynamicObject(ketsatroi8);
...
forward HackFlySamp(playerid);
public HackFlySamp(playerid)
{
    foreach(new i: Player)
    {
        ...
        if(Pos_x <= -0.800000  || Pos_y <= -0.800000 || Pos_z <= -0.800000 && anim == 1008)
        {
	        ...
	        new string[128];
	        GetPlayerName(playerid, name, sizeof(name));
	        format(string, sizeof(string), "[CANH BAO]: %s dang tinh nghi la su dung phan mem hack Fly.", name);
            if(PlayerInfo[i][pAdmin] >= 2)
    	    {
...
forward RaVien(playerid);
public RaVien(playerid)
{
    SetPVarInt(playerid, "TimeRaVien", GetPVarInt(playerid, "TimeRaVien")-1);
    ApplyAnimation(playerid, "BEACH", "bather", 4.0, 1, 0, 0, 0, 0, 1);
    new string[1280];
    format(string, sizeof(string), "~n~~n~~n~~n~~n~~n~~n~~n~~n~~w~%d giay con lai", GetPVarInt(playerid, "TimeRaVien"));
    GameTextForPlayer(playerid, string, 1100, 3);

    if(GetPVarInt(playerid, "TimeRaVien") > 0) SetTimerEx("RaVien", 1000, 0, "d", playerid);

    if(GetPVarInt(playerid, "TimeRaVien") <= 0)
    {
...
public DonGo(playerid)
{
    ...
    if(GetPVarInt(playerid, "DonGoTime") <= 0)
    {
        ...
        SetPlayerCheckpoint(playerid, -447.0288,-99.0795,64.8524, 5);
...
public TimeSV()
{
	...
	TextDrawSetString(TextDraw_1[3], string);

	format(string, sizeof string, "%s%d:%s%d", (hours < 10) ? ("0") : (""), hours, (minutes < 10) ? ("0") : (""), minutes, (seconds < 10) ? ("0") : (""), seconds);
	TextDrawSetString(TextDraw_1[4], string);
}

forward STATCC(playerid);
public STATCC(playerid)
{
    new idplayer[1280];
	format(idplayer,sizeof(idplayer),"ID: %d", playerid);
	PlayerTextDrawSetString(playerid,TextDraw_1_1[playerid][0],idplayer);

	new levelplayer[1280];
	format(levelplayer,sizeof(levelplayer),"Level: %d", PlayerInfo[playerid][pLevel]);
	PlayerTextDrawSetString(playerid,TextDraw_1_1[playerid][1],levelplayer);
}
...
public TinNhan()
{
    ...
    if(MessageID == 0)
    {
        for(new i = 0; i <= 500; i++)
        {
            if(IsPlayerConnected(i) && ChatBot[i] == 0)
            {
                format(string, sizeof(string),"Tin Nhan: Chao mung den voi may chu WOLRP.NET ROLEPLAY.");
    ...
    else if(MessageID == 1)
    {
        for(new i = 0; i <= 500; i++)
        {
            if(IsPlayerConnected(i) && ChatBot[i] == 0)
            {
                format(string, sizeof(string),"Tin Nhan: Neu ban dang thieu tien, ban co the lam cong viec Container.");
                SendClientMessageEx(i,COLOR_GREEN,string);
            }
        }
    }
    else if(MessageID == 2)
    {
        for(new i = 0; i <= 500; i++)
        {
            if(IsPlayerConnected(i) && ChatBot[i] == 0)
            {
                format(string, sizeof(string),"Tin Nhan: Neu ban khong biet duong thi hay [/timduong] de tim kiem noi ban muon /MAP nua nhe.");
                SendClientMessageEx(i,COLOR_GREEN,string);
            }
        }
    }
    else if(MessageID == 3)
    {
        for(new i = 0; i <= 500; i++)
        {
            if(IsPlayerConnected(i) && ChatBot[i] == 0)
            {
                format(string, sizeof(string),"Tin Nhan: Chuc ban co 1 ngay tot lanh.");
    ...
    else if(MessageID == 4)
    {
        for(new i = 0; i <= 500; i++)
        {
            if(IsPlayerConnected(i) && ChatBot[i] == 0)
            {
                format(string, sizeof(string),"Tin Nhan: Neu ban thac mac gi ban co the [/newb] hoac [/yeucautrogiup] de duoc tro giup.");
                SendClientMessageEx(i,COLOR_GREEN,string);
            }
        }
    }
    else if(MessageID == 5)
    {
        for(new i = 0; i <= 500; i++)
        {
            if(IsPlayerConnected(i) && ChatBot[i] == 0)
            {
                format(string, sizeof(string),"Tin Nhan: Neu ban thay hack/cheat/cleo hay su dung [/baocao] -> hacking.");
                SendClientMessage(i,COLOR_GREEN,string);
            }
        }
    }
    else if(MessageID == 6)
    {
        for(new i = 0; i <= 500; i++)
        {
            if(IsPlayerConnected(i) && ChatBot[i] == 0)
            {
                format(string, sizeof(string),"Tin Nhan: Neu ban khong biet duong hoac lac duong thi hay [/dichvu taxi] de ho dua ban di.");
                SendClientMessageEx(i,COLOR_GREEN,string);
            }
        }
    }
    else if(MessageID == 7)
    {
        for(new i = 0; i <= 500; i++)
        {
            if(IsPlayerConnected(i) && ChatBot[i] == 0)
            {
                format(string, sizeof(string),"Tin Nhan: Hay RolePlay theo cach cua ban.");
                SendClientMessageEx(i,COLOR_GREEN,string);
            }
        }
    }
...
forward DLTextDraw(playerid, value);
public DLTextDraw(playerid, value)
{
	if(value == 1)
	{
		SelectTextDraw(playerid, 1677721690);
		DLCheck(playerid);
		for(new i=0; i <= 24; i++) TextDrawShowForPlayer(playerid, DLTextDraws[i][playerid]);
	}
	else if(value == 2)
	{
	    CancelSelectTextDraw(playerid);
		for(new i=0; i <= 24; i++) TextDrawHideForPlayer(playerid, DLTextDraws[i][playerid]);
	}
	return 1;
}

forward DLCheck(playerid);
public DLCheck(playerid)
{
	if(PlayerInfo[playerid][DLRewardBox][1] == 0 && PlayerInfo[playerid][pDonateRank] >= 1)
	{
	    TextDrawSetString(DLTextDraws[15][playerid], "ld_chat:thumbup");
	    TextDrawShowForPlayer(playerid, DLTextDraws[15][playerid]);
	}
	else
	{
	    TextDrawSetString(DLTextDraws[15][playerid], "ld_chat:thumbdn");
	    TextDrawShowForPlayer(playerid, DLTextDraws[15][playerid]);
	}
	if(PlayerInfo[playerid][DLRewardBox][2] == 0 && PlayerInfo[playerid][pDonateRank] >= 2)
	{
	    TextDrawSetString(DLTextDraws[16][playerid], "ld_chat:thumbup");
	    TextDrawShowForPlayer(playerid, DLTextDraws[16][playerid]);
	}
	else
	{
	    TextDrawSetString(DLTextDraws[16][playerid], "ld_chat:thumbdn");
	    TextDrawShowForPlayer(playerid, DLTextDraws[16][playerid]);
	}
	return 1;
}
