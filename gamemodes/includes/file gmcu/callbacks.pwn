...
public OnPlayerWeaponShot(playerid, weaponid, hittype, hitid, Float:fX, Float:fY, Float:fZ)
{
	...
	if(weaponid != 38)
    {
        ...
        {
        	new string[128];
			format(string, sizeof(string), "{AA3333}AdmWarning{FFFF00}: %s (ID %d) dang su dung Hack Rapid (( /spec %d )).", GetPlayerNameEx(playerid), playerid, playerid);
...
{
  	new doibung[1280];
    format(doibung,sizeof(doibung),"%d", PlayerInfo[playerid][pDoiBung]);
	PlayerTextDrawSetString(playerid, PlayernDPDz[playerid][0],doibung);

	new khatnuoc[1280];
	format(khatnuoc,sizeof(khatnuoc),"%d", PlayerInfo[playerid][pKhatNuoc]);
	PlayerTextDrawSetString(playerid, PlayernDPDz[playerid][1],khatnuoc);
	if(!IsPlayerInRangeOfPoint(playerid, 50.0, -304.4369,1894.9891,29.8929) || !IsPlayerInRangeOfPoint(playerid, 50.0, -295.9042,1894.9121,29.8929) || !IsPlayerInRangeOfPoint(playerid, 50.0, -287.1133,1894.6259,29.8929))
	{
	    if(PlayerInfo[playerid][pJailTime] > 0)
		{
			...
			SendClientMessageEx(playerid, COLOR_GRAD1, "Ban da duoc dich chuyen ve nha tu , do nghi van bug.");
			return 1;
		}
	}
    ...
    if(PlayerInfo[playerid][pAdmin] < 2 && GetPVarInt(playerid, "spam1") == 0)
    {
		...
		{
			format(string,sizeof(string),"{AA3333}Anti Cheat/Hacking{FFFF00}: %s (ID: %d) co the dang su dung slap hack hoac surfly onfoot.",GetPlayerNameEx(playerid), playerid);
			ABroadCast(COLOR_YELLOW , string, 2);
	...
	for(new Sz; Sz < MAX_SZ; Sz++)
 	{
  		if(IsPlayerInRangeOfPoint(playerid, SafeZoneInfo[Sz][szSize], SafeZoneInfo[Sz][szPosX], SafeZoneInfo[Sz][szPosY], SafeZoneInfo[Sz][szPosZ]))
  		{
  		    if(!IsACop(playerid) || IsAHitman(playerid))
			{
   				if(EnterSafeZones[playerid] < 0)
	            {
	                ...
	                SendClientMessageEx(playerid, COLOR_WHITE, "{00ff00}Ban da tro lai khu vuc an toan{ffffff} {ff3300}(SAFEZONE){ffffff}.");
	            }
  		...
  		if(Sz == EnterSafeZones[playerid] && EnterSafeZones[playerid] >= 0)
        {
            if(!IsPlayerInRangeOfPoint(playerid, SafeZoneInfo[Sz][szSize], SafeZoneInfo[Sz][szPosX], SafeZoneInfo[Sz][szPosY], SafeZoneInfo[Sz][szPosZ]))
            {
                ...
                SendClientMessageEx(playerid, COLOR_WHITE, "{ffff00}Ban da roi khoi khu vuc an toan{ffffff} {ff3300}(SAFEZONE){ffffff}.");
            }
            break;
        }
 	}
...
public OnPlayerClickTextDraw(playerid, Text:clickedid)
{
    ...
    if(GetPVarInt(playerid, "NHANQUA_") == 1)
    {
		if(clickedid == DLTextDraws[12][playerid])
		{
		...
		else if(clickedid == DLTextDraws[13][playerid])
		{
			PlayerPlaySound(playerid, 1145, 0.0, 0.0, 0.0);
		...
		else if(clickedid == DLTextDraws[14][playerid])
		{
			PlayerPlaySound(playerid, 1145, 0.0, 0.0, 0.0);
		...
		else if(clickedid == DLTextDraws[2][playerid])
		{
		    PlayerPlaySound(playerid, 1145, 0.0, 0.0, 0.0);
		    new d, m, y;
			GetDate_V(d, m, y);
				...
				if(GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
				{
			        if(GetPlayerState(issuerid) == PLAYER_STATE_DRIVER)
			        {
			            if(weaponid == 49 || weaponid == 50)
			            {
							...
							GameTextForPlayer(issuerid, "~r~KHONG CR NGUOI KHAC THANG LOZ", 5000, 3);
				            ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
							return 1;
						}
					}
...
public OnPlayerGiveDamage(playerid, damagedid, Float:amount, weaponid)
{
	...
	for(new Sz; Sz < MAX_SZ; Sz++)
    {
    	new Float:php;
		if(IsPlayerInRangeOfPoint(playerid, SafeZoneInfo[Sz][szSize], SafeZoneInfo[Sz][szPosX], SafeZoneInfo[Sz][szPosY], SafeZoneInfo[Sz][szPosZ]))
		{// Neu nguoi choi o trong khu vuc an toan
			if(!IsACop(playerid) || IsAHitman(playerid))
			{
  		...
  		if(!IsPlayerInRangeOfPoint(playerid, SafeZoneInfo[Sz][szSize], SafeZoneInfo[Sz][szPosX], SafeZoneInfo[Sz][szPosY], SafeZoneInfo[Sz][szPosZ]))
		{//neu nguoi choi tan cong o ngoai khu vuc an toan va nguoi choi bi tan cong o trong khu vuc an toan
...
// chu code hd ma tai sao nhu vay
public OnPlayerConnect(playerid) {
	...
	GetPlayerIp(playerid, playerip, sizeof(playerip));
	new stringcc[1280];
	format(stringcc, sizeof(stringcc), "{f4eded}WOLRP.NET: %s dang dang nhap, {fd32ff}IP Address: %s [ID: %d]", name, playerip, playerid);
	ABroadCast(COLOR_YELLOW , stringcc, 2);
	...
	GetPlayerClientID(playerid, PlayerInfo[playerid][pGPCI], 40);

	SendClientMessage(playerid, -1, "Ban khong the ket noi bang PC");

	player_mobile[playerid] = false;

	if(strfind(PlayerInfo[playerid][pGPCI], "5638413348335738345A4536524D4A524539334B") != -1)

	SendClientMessage(playerid, -1, "Ban da ket noi voi mobile");
	...
	logincheck[playerid] = SetTimerEx("LoginCheck", 99999999, 0, "i", playerid);
	
	SetTimerEx("LoginCheckEx", 500000000, 0, "i", playerid);
...
public OnPlayerDisconnect(playerid, reason)
{
	...
	if(PlayerInfo[playerid][Robbing] == 1)
	{
		...
		new string[128];
		format(string,sizeof(string),"** Ban da chet va khong the nhan duoc so tien này $%d cua ban",PlayerInfo[playerid][CashLoaded]);
...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	if(RobStatus[Setup] == 1 && RobStatus[BeingRobbed] == 0 && RobStatus[RobberID1] == playerid)
 	{
  		RobStatus[Setup] = 0;
    	SendClientMessage(RobStatus[RobberID1], COLOR_YELLOW, "** Chi huy vu cuop da chet, vu cuop ket thuc **");
     	SendClientMessage(RobStatus[RobberID2], COLOR_YELLOW, "** Chi huy vu cuop da chet, vu cuop ket thuc **");
      	SendClientMessage(RobStatus[RobberID3], COLOR_YELLOW, "** Chi huy vu cuop da chet, vu cuop ket thuc **");
       	SendClientMessage(RobStatus[RobberID4], COLOR_YELLOW, "** Chi huy vu cuop da chet, vu cuop ket thuc **");
        SendClientMessage(RobStatus[RobberID5], COLOR_YELLOW, "** Chi huy vu cuop da chet, vu cuop ket thuc **");
        SendClientMessageToAll(COLOR_YELLOW, "** Chi huy vu cuop da chet, canh sat dang truy lung cac ten con lai**");
        RobStatus[RobberID1] = 255;
        RobStatus[RobberID2] = 255;
        RobStatus[RobberID3] = 255;
        RobStatus[RobberID4] = 255;
	...
	if(PlayerInfo[playerid][Robbing] == 1)
	{
		...
		new string[128];
		format(string,sizeof(string),"** Ban da chet, canh sat da thu hoi %d $ ban vua cuop duoc ",PlayerInfo[playerid][CashLoaded]);
		SendClientMessage(playerid, COLOR_YELLOW, string);
		PlayerInfo[playerid][CashLoaded] = 0;
	    //dat o day
		PlayerInfo[playerid][Robbing] = 0;
	...
	{
	    new szMessage[128];
		format(szMessage, sizeof(szMessage), "[KILL] Ban da giet chet %s", GetPlayerNameEx(playerid));
		SendClientMessageEx(killerid, COLOR_YELLOW, szMessage);

		if(IsACop(playerid) && PlayerInfo[playerid][pDuty] == 1)
		{
		    PlayerInfo[killerid][pMats] += 1000;
		    PlayerInfo[killerid][pCash] += 5000;
		    SendClientMessage(killerid, COLOR_YELLOW, "Ban nhan duoc 1000 vat lieu va 5000$ , khi tieu diet duoc canh sat.");
		}

		new szMessagecc[128];
		format(szMessagecc, sizeof(szMessagecc), "[DEATH] Ban da bi giet chet boi %s", GetPlayerNameEx(killerid));
...
public OnPlayerSpawn(playerid)
{
	...
	TextDrawShowForPlayer(playerid,nDPDz[0]);
	TextDrawShowForPlayer(playerid,nDPDz[1]);

	PlayerTextDrawShow(playerid, PlayernDPDz[playerid][0]);
	PlayerTextDrawShow(playerid, PlayernDPDz[playerid][1]);
	
	if(GetPVarInt(playerid, "NotAndroid") != 0 && PlayerInfo[playerid][pAdmin] < 2)
    {
	...
	hienantihackfly[playerid] = SetTimerEx("HackFlySamp", 100, true, "i", playerid);
...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	{
	   if(IsPlayerInRangeOfPoint(playerid, 6.0, -2350.6536, 1299.6273, 16.2156))
	   {
	       DisablePlayerCheckpoint(playerid);
	       new pizzacash = 12000+random(1);
	       PlayerInfo[playerid][pCash] += pizzacash;
	       new string[1280];
	       format(string,sizeof(string),"~g~Chuc mung~w~: ban nhan duoc %d tu viec giao thanh cong Pizza",pizzacash);
	       GameTextForPlayer(playerid,string,10000,5);
	       PizzaCP[playerid] = 0;
	       PlayerInfo[playerid][pSLPizza] += 1;
	       return 1;
       }else SendClientMessageEx(playerid, COLOR_GREY, "Loi cong viec, ban vui long thu lai.");
    }
     
    if(GetPVarInt(playerid, "GiaoXang_") == 1)
    {
        if(IsPlayerInRangeOfPoint(playerid, 3.0, -2407.1641,976.2516,45.2969))
        {
		    ...
		    if(XeGiaoXang(vehicleid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		    {
				...
				SetPVarInt(playerid, "LayXangTime1", 20);
    ...
    {
        if(IsPlayerInRangeOfPoint(playerid, 3.0, -2090.4983,95.7303,35.3203))
        {
	        new vehicleid = GetPlayerVehicleID(playerid);
		    if(XeGiaoXang(vehicleid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		    {
				...
				//_____________________________________________________________//
				SetPVarInt(playerid, "LayXangTime2", 20);
    ...
    {
        if(IsPlayerInRangeOfPoint(playerid, 3.0, -2120.8640,-264.0453,35.3203))
        {
	        new vehicleid = GetPlayerVehicleID(playerid);
		    if(XeGiaoXang(vehicleid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		    {
				...
				//_____________________________________________________________//
				SetPVarInt(playerid, "LayXangTime3", 20);
    ...
    {
        if(IsPlayerInRangeOfPoint(playerid, 3.0, -2459.7332,786.1253,35.1719))
        {
	        new vehicleid = GetPlayerVehicleID(playerid);
		    if(XeGiaoXang(vehicleid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		    {
				...
				//_____________________________________________________________//
				SetPVarInt(playerid, "LayXangTime4", 20);
    ...
    if(GetPVarInt(playerid, "GiaoXang_") == 5)
    {
        if(IsPlayerInRangeOfPoint(playerid, 3.0, -2459.7332,786.1253,35.1719))
        {
		    ...
		    if(XeGiaoXang(vehicleid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		    {
				...
				//_____________________________________________________________//
				SetPVarInt(playerid, "LayXangTime5", 20);
		        SetTimerEx("LayXang5", 1000, 0, "d", playerid);
		        TogglePlayerControllable(playerid, 0);
		        SendClientMessageEx(playerid, COLOR_GREY, "Ban dang giao xang , vui long doi trong giay lat.");
		        return 1;
	        }else SendClientMessageEx(playerid, COLOR_GREY, "Ban khong o tren xe van chuyen xang dau.");
    ...
    {
        if(IsPlayerInRangeOfPoint(playerid, 3.0, -1676.3558,412.7199,7.1797))
        {
	        new vehicleid = GetPlayerVehicleID(playerid);
		    if(XeGiaoXang(vehicleid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		    {
	            ...
	            PlayerInfo[playerid][pSLGiaoXang] += 1;
				return 1;
			}else SendClientMessageEx(playerid, COLOR_GREY, "Ban khong o tren xe van chuyen xang dau.");
		}
    }
    ...
    if(GetPVarInt(playerid,"GiaoGo") == 1)
	{
        ...
        PlayerInfo[playerid][pSLLumber] += 1;
        DangCuaCay[playerid] = 0;
		DeletePVar(playerid, "DonGoTime");
		DeletePVar(playerid, "DonGo");

        SoLanCuaCay[playerid] += 1;

        if(SoLanCuaCay[playerid] == 10)
        {
            TimeResetCuaCay[playerid] = 300000;
        }
    ...
    if(GetPVarInt(playerid, "GiaoTraiCay_") == 1)
    {
        if(IsPlayerInRangeOfPoint(playerid, 5.0,-1790.3539,1429.1788,7.1875))
		{
	        DisablePlayerCheckpoint(playerid);
			...
			new money = 25000+random(1);
			PlayerInfo[playerid][pCash] += money;

			PlayerInfo[playerid][pSLGiaoTraiCay] += 1;

			new string[1280];
			format(string, sizeof(string), "Ban da giao thanh cong mot thung trai cay , va nhan duoc %d", money);
			SendClientMessageEx(playerid, COLOR_YELLOW, string);
	        return 1;
		}else SendClientMessageEx(playerid,COLOR_YELLOW, " loi cong viec rui . bug nua admin banned ");
    }
    if(GetPVarInt(playerid,"nhantien") != 0)
	{
		if(PlayerInfo[playerid][Robbing] == 1)
		{
			...
			new stringa[128];
			format(stringa,sizeof(stringa),"Ban da den diem an toan và tui' tien` cua ban la` $%d.",PlayerInfo[playerid][CashLoaded]);
    ...
    if(GetPVarInt(playerid,"GiaoBaoDz") == 1)
    {
        PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
	    DisablePlayerCheckpoint(playerid);

        new gb = 1+random(2);
		PlayerGB[playerid][pDaMuaGB] -= gb;

        new money = 5000+random(1);
        new str[2460], str1[1280], str2[1280], str3[1280], str4[1280], name[1280];
		format(str1, sizeof(str), "Ban da giao bao thanh cong!");
	    format(str2, sizeof(str), "   + SO LUONG BAO HIEN TAI : %d - TONG SO TIEN LA : %d", PlayerGB[playerid][pDaMuaGB], (PlayerGB[playerid][pToBao] - (PlayerGB[playerid][pToBao] / 3)) + money);
	    format(str3, sizeof(str), "   + BAN NHAN DUOC {00ff00}%d{FFFFFF} TU VIEC GIAO BAO", money);
	    format(str4, sizeof(str), "Neu ban muon tiep tuc giao bao vui long nhan ' /giaobao '.");
	    format(str, sizeof(str), "%s\n\n%s\n\n%s\n\n%s\n\n%s\n\n%s", str1,str2,str3,str4);
	    format(name, sizeof(name), "GIAO BAO");
		ShowPlayerDialog(playerid, DIALOG_NOTHING, DIALOG_STYLE_MSGBOX, name, str, "Dong y", "");

		PlayerInfo[playerid][pCash] += (PlayerGB[playerid][pToBao] - (PlayerGB[playerid][pToBao] / 3)) + money;

		PlayerInfo[playerid][pXP] += 10;

		SetPVarInt(playerid, "GiaoBaoDz", 0);
		DeletePVar(playerid, "GiaoBaoDz");

		SetPVarInt(playerid, "HienDangGiao", 0);
		DeletePVar(playerid, "HienDangGiao");
        return 1;
    }
    ...
    if(CP[playerid] == 252006)
    {
        CP[playerid] = 0;
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
    	SendClientMessage(playerid, COLOR_GREY, " Ban da den noi!");
        DisablePlayerCheckpoint(playerid);
    }
	...
	if(GetPVarInt(playerid, "TruckDeliver") > 0 && gPlayerCheckpointStatus[playerid] != CHECKPOINT_RETURNTRUCK)
	{
		...
		{
			SetPlayerCheckpoint(playerid, -1547.5298,124.7811,3.5547, 5); // 2413.7336,-2137.5742,13.2971
			GameTextForPlayer(playerid, "~w~Dia diem ~r~OCEAN DOCKS", 5000, 1);
			SendClientMessageEx(playerid, COLOR_WHITE, "HINT: Quay tro lai OCEAN DOCKS (diem checkpoint tren radar).");
		}
		else
		{
	...
	if(gPlayerCheckpointStatus[playerid] == CHECKPOINT_DELIVERY)
	{
		...
		new string[128];
		format(string, sizeof(string), "* Ban da gui %s den %s. Tra lai xe tai toi OCEAN DOCKS de nhan tien.", GetInventoryType(business), Businesses[business][bName]);
		...
		gPlayerCheckpointStatus[playerid] = CHECKPOINT_RETURNTRUCK;
		SetPlayerCheckpoint(playerid, -1547.5298,124.7811,3.5547, 5);
		GameTextForPlayer(playerid, "~w~Dia diem ~r~OCEAN DOCKS", 5000, 1);
		SendClientMessageEx(playerid, COLOR_WHITE, "HINT: Quay lai OCEAN DOCKS (diem checkpoint tren radar).");
	...
	if(GetPVarInt(playerid, "Pizza") > 0 && IsPlayerInRangeOfPoint(playerid, 5.0, HouseInfo[GetPVarInt(playerid, "Pizza")][hExteriorX], HouseInfo[GetPVarInt(playerid, "Pizza")][hExteriorY], HouseInfo[GetPVarInt(playerid, "Pizza")][hExteriorZ]) && GetPlayerInterior(playerid) == HouseInfo[GetPVarInt(playerid, "Pizza")][hExtIW] && GetPlayerVirtualWorld(playerid) == HouseInfo[GetPVarInt(playerid, "Pizza")][hExtVW])
	{
		...
		format(string, sizeof(string), "Ban da van chuyen banh pizza toi dich! Ban nhan duoc $%d tien banh.", (GetPVarInt(playerid, "pizzaTimer") * 500));
		Misc_Save();
		GivePlayerCash(playerid, floatround((GetPVarInt(playerid, "pizzaTimer") * 500), floatround_round));
		SendClientMessageEx(playerid, COLOR_WHITE, string);
	...
	else
	{
		switch (gPlayerCheckpointStatus[playerid])
		{
			...
			{
			    if(IsPlayerInRangeOfPoint(playerid, 6, -1603.4996,69.5493,3.5547) || IsPlayerInRangeOfPoint(playerid, 6, 2098.6543,-104.3568,-0.4820))
			    {
				    new vehicleid = GetPlayerVehicleID(playerid);
			...
			{
			    if(!IsPlayerInRangeOfPoint(playerid, 6, -1547.5298,124.7811,3.5547) && !IsPlayerInRangeOfPoint(playerid, 6, 2098.6543,-104.3568,-0.4820))
				...
				if(level >= 0 && level <= 50) payment = 10000;
				else if(level >= 51 && level <= 100) payment = 20000;
				else if(level >= 101 && level <= 200) payment = 30000;
				else if(level >= 201 && level <= 400) payment = 40000;
				else if(level >= 401) payment = 50000;
				...
				{
				    distancepay = floatmul(GetDistance(-1603.4996,69.5493,3.5547, TruckerDropoffs[route][PosX], TruckerDropoffs[route][PosY], TruckerDropoffs[route][PosZ]), 1.5);
				}
				...
				if(TruckDeliveringTo[vehicleid] != INVALID_BUSINESS_ID) {
				    ...
				    PlayerInfo[playerid][pSLTrucker] += 1;
				}
				else {
					...
					if(truckdeliver == 5) // Weapons
					{
						if(PlayerInfo[playerid][pConnectHours] >= 2 && PlayerInfo[playerid][pWRestricted] <= 0)
						{
							if(level >= 0 && level <= 50)
							{
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban nhan duoc mien phi Baseball Bat cho viec van chuyen vu khi trai phep.");
							...
							{
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban nhan duoc mien phi Golf club cho viec van chuyen vu khi trai phep.");
                                SendClientMessageEx(playerid, COLOR_YELLOW, "* Khong duoc su dung vu khi duoc thuong cho muc dich xau nhu KOS/DM nonRP, neu vi pham se bi han che vu khi.");
								GivePlayerValidWeapon(playerid, 2, 60000);
							}
							else if(level >= 101 && level <= 200)
							{
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban nhan duoc mien phi Katana cho viec van chuyen vu khi trai phep.");
								SendClientMessageEx(playerid, COLOR_YELLOW, "* Khong duoc su dung vu khi duoc thuong cho muc dich xau nhu KOS/DM nonRP, neu vi pham se bi han che vu khi.");
								GivePlayerValidWeapon(playerid, 8, 60000);
							}
							else if(level >= 201 && level <= 400)
							{
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban nhan duoc mien phi 9mm Pistol cho viec van chuyen vu khi trai phep.");
								SendClientMessageEx(playerid, COLOR_YELLOW, "* Khong duoc su dung vu khi duoc thuong cho muc dich xau nhu KOS/DM nonRP, neu vi pham se bi han che vu khi.");
								GivePlayerValidWeapon(playerid, 22, 60000);
							}
							else if(level >= 401)
							{
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban nhan duoc mien phi Silenced pistol cho viec van chuyen vu khi trai phep.");
								SendClientMessageEx(playerid, COLOR_YELLOW, "* Khong duoc su dung vu khi duoc thuong cho muc dich xau nhu KOS/DM nonRP, neu vi pham se bi han che vu khi.");
					...
					if(truckdeliver == 7) // Illegal materials
					{
						if(level >= 0 && level <= 50)
						{
		                    SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban da nhan duoc 500 cho viec van chuyen vat lieu bat hop phap.");
						...
						{
		                    SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban da nhan duoc 1500 cho viec van chuyen vat lieu bat hop phap.");
							PlayerInfo[playerid][pMats] += 1500;
						}
						else if(level >= 201 && level <= 400)
						{
		                    SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban da nhan duoc 1750 cho viec van chuyen vat lieu bat hop phap.");
							PlayerInfo[playerid][pMats] += 1750;
						}
						else if(level >= 401)
						{
		                    SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban da nhan duoc 2000 cho viec van chuyen vat lieu bat hop phap.");
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    {
        if(IsPlayerInRangeOfPoint(playerid, 100.0, 607.8454,867.6566,-40.2512))
		{
		    cmd_daoda(playerid, "");
		}
    }
    ...
    if(PRESSED(KEY_WALK))
    {
        new carid = GetPlayerVehicleID(playerid);
	 	new closestcar = GetClosestCar(playerid, carid);
	  	if(Trash_[playerid] == 1)
	  	{
			if(PlayerInfo[playerid][pJob] == 26 || PlayerInfo[playerid][pJob2] == 26)
			{
				if(IsPlayerInRangeOfVehicle(playerid, closestcar, 15.0) && IsADonRacCar(closestcar))
				{
				    RemovePlayerAttachedObject(playerid, ATTACHMENT_INDEX);
				    SoLuongRac[playerid] += 1;
				    SendClientMessage(playerid, -1, "Ban da bo 1 bao rac len xe. ");
					SendClientMessage(playerid, -1, "Su dung: /layrac, de tiep tuc cong viec. ");
					new string[128];
					format(string, sizeof(string), "Hien tai ban co : %d bao rac. [Gia ban : 500$ / 1 Bao Rac]", SoLuongRac[playerid]);
					SendClientMessage(playerid, -1, string);
					Trash_[playerid] = 0;
					DangLayRac[playerid] = 0;
					return 1;
				}else SendClientMessageEx(playerid, COLOR_WHITE, "Ban can phai o gan chiec xe cho rac cua minh.");
			}
		}
    }
    ...
    if(PRESSED(KEY_YES))
    {
		...
		{
            ShowPlayerDialog(playerid, DIALOG_POTCRACK, DIALOG_STYLE_LIST, "{ffffff} WOLRP.NET | BAN POT / CRACK", "SELL POT\t1POT = 1,000$\nSELL CRACK\t1 CRACK = 1,000$", "Dong y", "Huy bo");
		}
    }
    
    if(newkeys & KEY_YES || PRESSED(KEY_WALK))
    {
        if(IsAtMayDauKhi(playerid))
        {
        	cmd_suangay(playerid, "");
        }
    }
        ...
        if (IsPlayerSteppingInVehicle[playerid] > -1)
        {
            for(new i = 0; i < MAX_PLAYERS; i++)
            {
                if (GetPlayerVehicleID(i) == IsPlayerSteppingInVehicle[playerid] && i != playerid)
                {
                    ...
                    GameTextForPlayer(playerid,"~r~Ban khong duoc NJ nguoi khac",5000,4);  // ong kiem nut alt dung r ma no ko nhan nut Y thu xem
	      ...
	      if(PlantedC4OnVault[playerid] == 1)
     	  {
			...
			new string[126];
			format(string, sizeof(string), "** KET SAT CUA NGAN HANG nhà NUOC' DA~ BI PHA' HUY! **");
			...
			SendClientMessageToAll(COLOR_ORANGE, "** 1 TIENG NO VANG TROI..RUNG DONG CA thành PHO' **");
			SendClientMessageToAll(COLOR_ORANGE, "** 1 TIENG NO VANG TROI..RUNG DONG CA thành PHO' **");
...
public OnPlayerText(playerid, text[])
{
	...
	switch(PlayerInfo[playerid][pAccent])
	{
		...
		case 26: accent = "{676C70}(WOLRP){FFFFFF} ";
