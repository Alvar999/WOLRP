...
public OnQueryFinish(resultid, extraid, handleid)
{
	...
	switch(resultid)
	{
		...
		case LOADUSERDATA_THREAD:
		{
			if(IsPlayerConnected(extraid))
			{
				...
				for(new row;row < rows;row++)
				{
					...
					cache_get_field_content(row,  "DiemDanh", szResult, MainPipeline); PlayerInfo[extraid][pDiemDanh] = strval(szResult);
					...
					cache_get_field_content(row,  "DLDate", szResult, MainPipeline); PlayerInfo[extraid][DLDate] = strval(szResult);
					cache_get_field_content(row,  "DLRewardBox0", szResult, MainPipeline); PlayerInfo[extraid][DLRewardBox][0] = strval(szResult);
					cache_get_field_content(row,  "DLRewardBox1", szResult, MainPipeline); PlayerInfo[extraid][DLRewardBox][1] = strval(szResult);
					cache_get_field_content(row,  "DLRewardBox2", szResult, MainPipeline); PlayerInfo[extraid][DLRewardBox][2] = strval(szResult);
					
					cache_get_field_content(row,  "TKNganHang", szResult, MainPipeline); PlayerInfo[extraid][pTKNganHang] = strval(szResult);
					cache_get_field_content(row,  "CanSa", szResult, MainPipeline); PlayerInfo[extraid][pCanSa] = strval(szResult);
					cache_get_field_content(row,  "KimCuong", szResult, MainPipeline); PlayerInfo[extraid][pKimCuong] = strval(szResult);
					
					cache_get_field_content(row,  "GiayPhep", szResult, MainPipeline); PlayerInfo[extraid][pGiayPhep] = strval(szResult);
					
					cache_get_field_content(row,  "SLTrucker", szResult, MainPipeline); PlayerInfo[extraid][pSLTrucker] = strval(szResult);
					cache_get_field_content(row,  "SLPizza", szResult, MainPipeline); PlayerInfo[extraid][pSLPizza] = strval(szResult);
					cache_get_field_content(row,  "SLDaoDa", szResult, MainPipeline); PlayerInfo[extraid][pSLDaoDa] = strval(szResult);
					cache_get_field_content(row,  "SLGiaoXang", szResult, MainPipeline); PlayerInfo[extraid][pSLGiaoXang] = strval(szResult);
					cache_get_field_content(row,  "SLLumber", szResult, MainPipeline); PlayerInfo[extraid][pSLLumber] = strval(szResult);
					cache_get_field_content(row,  "SLGiaoTraiCay", szResult, MainPipeline); PlayerInfo[extraid][pSLGiaoTraiCay] = strval(szResult);
		...
		case IPBAN_THREAD:
		{
		    if(rows > 0)
			{
				SendClientMessage(extraid, COLOR_RED, "IP cua ban bi cam truy cap, de mo khoa vui long lien he Finn Trong Discord De mo khoa");
				SetTimerEx("KickEx", 1000, 0, "i", extraid);
			}
			else
			{
		...
		case MAIN_REFERRAL_THREAD:
		{
		    ...
		    if(newrows == 0)
		    {
		        ...
		        ShowPlayerDialog(extraid, REGISTERREF, DIALOG_STYLE_INPUT, "{FF0000}Loi - Nguoi choi khong hop le", "Khong co nguoi choi dang ky tai may chu WOLRP voi ten nhu vay.\nVui long nhap ten day du nguoi choi da gioi thieu ban.\nVi du: Beo_cu, Long_Trieu", "Dong y", "Huy bo");
			}
			else {
		...
		case REWARD_REFERRAL_THREAD:
		{
			...
			if(newrows != 0)
			{
			    SendClientMessageEx(extraid, COLOR_YELLOW, "Nguoi choi da gioi thieu ban tham gia khong ton tai tren he thong WOLRP, vi vay ho se khong nhan duoc credits gioi thieu");
			}
		}
...
stock g_mysql_SaveAccount(playerid)
{
    ...
    SavePlayerInteger(query, GetPlayerSQLId(playerid), "DiemDanh", PlayerInfo[playerid][pDiemDanh]);
    ...
    // Phan thuong hang ngay
	SavePlayerInteger(query, GetPlayerSQLId(playerid), "DLDate", PlayerInfo[playerid][DLDate]);
	SavePlayerInteger(query, GetPlayerSQLId(playerid), "DLRewardBox0", PlayerInfo[playerid][DLRewardBox][0]);
	SavePlayerInteger(query, GetPlayerSQLId(playerid), "DLRewardBox1", PlayerInfo[playerid][DLRewardBox][1]);
	SavePlayerInteger(query, GetPlayerSQLId(playerid), "DLRewardBox2", PlayerInfo[playerid][DLRewardBox][2]);
    
    SavePlayerInteger(query, GetPlayerSQLId(playerid), "TKNganHang", PlayerInfo[playerid][pTKNganHang]);
    SavePlayerInteger(query, GetPlayerSQLId(playerid), "CanSa", PlayerInfo[playerid][pCanSa]);
    
    SavePlayerInteger(query, GetPlayerSQLId(playerid), "KimCuong", PlayerInfo[playerid][pKimCuong]);
    
    SavePlayerInteger(query, GetPlayerSQLId(playerid), "GiayPhep", PlayerInfo[playerid][pGiayPhep]);
    
    SavePlayerInteger(query, GetPlayerSQLId(playerid), "SLTrucker", PlayerInfo[playerid][pSLTrucker]);
    SavePlayerInteger(query, GetPlayerSQLId(playerid), "SLPizza", PlayerInfo[playerid][pSLPizza]);
    SavePlayerInteger(query, GetPlayerSQLId(playerid), "SLDaoDa", PlayerInfo[playerid][pSLDaoDa]);
    SavePlayerInteger(query, GetPlayerSQLId(playerid), "SLGiaoXang", PlayerInfo[playerid][pSLGiaoXang]);
    SavePlayerInteger(query, GetPlayerSQLId(playerid), "SLLumber", PlayerInfo[playerid][pSLLumber]);
    SavePlayerInteger(query, GetPlayerSQLId(playerid), "SLGiaoTraiCay", PlayerInfo[playerid][pSLGiaoTraiCay]);
...
stock SaveBusiness(id)
{
	...
	`LevelProgress` = %d, `AutoSale` = %d, `OrderAmount` = %d, `OrderBy` = '%s', `OrderState` = %d, `TotalSales` = %d, ",
	query,
	...
	Businesses[id][bLevelProgress], Businesses[id][bAutoSale], Businesses[id][bOrderAmount], g_mysql_ReturnEscaped(Businesses[id][bOrderBy], MainPipeline), Businesses[id][bOrderState], Businesses[id][bTotalSales]);
