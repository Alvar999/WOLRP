forward NewbieNames(playerid);
public NewbieNames(playerid)
{
	if(PlayerInfo[playerid][pLevel] == 1)
	{
		if(PlayerInfo[playerid][pLevel] == 1)
		{
	        SetPlayerChatBubble(playerid, "NEWBIE", COLOR_GREEN, 20.0, 2000);
		}
	}
}
...
public OnPlayerLoad(playerid)
{
	...
	SetTimerEx("NewbieNames",1000,true,"i",playerid);
    SetTimerEx("STATCC",1000,true,"i",playerid);
	...
	SetTimerEx("CheckNgayDiemDanh", 850*1, false, "i", playerid);
	SetTimerEx("ResetCongViec", 850*1, false, "i", playerid);
    format(string, sizeof(string), "Chao mung ban den voi server - %s", GetPlayerNameEx(playerid));
	TextDrawShowForPlayer(playerid, TextDraw_1[0]);
	TextDrawShowForPlayer(playerid, TextDraw_1[1]);
	TextDrawShowForPlayer(playerid, TextDraw_1[2]);
	TextDrawShowForPlayer(playerid, TextDraw_1[3]);
	TextDrawShowForPlayer(playerid, TextDraw_1[4]);
	TextDrawShowForPlayer(playerid, TextDraw_1[5]);
	TextDrawShowForPlayer(playerid, TextDraw_1[6]);
	TextDrawShowForPlayer(playerid, TextDraw_1[7]);
	TextDrawShowForPlayer(playerid, TextDraw_1[8]);
	PlayerTextDrawShow(playerid,TextDraw_1_1[playerid][0]);
	PlayerTextDrawShow(playerid,TextDraw_1_1[playerid][1]);
...
public ResetCongViec(playerid)
{
	new d, m, y;
	GetDate_V(d, m, y);
	if(d != PlayerInfo[playerid][DLDate])
	{
	    PlayerInfo[playerid][pSLTrucker] = 0;
	    PlayerInfo[playerid][pSLPizza] = 0;
	    PlayerInfo[playerid][pSLGiaoXang] = 0;
	    PlayerInfo[playerid][pSLGiaoTraiCay] = 0;
	    PlayerInfo[playerid][pSLLumber] = 0;
	    PlayerInfo[playerid][DLRewardBox][0] = 0;
	    PlayerInfo[playerid][DLRewardBox][1] = 0;
	    PlayerInfo[playerid][DLRewardBox][2] = 0;
	    SendClientMessage(playerid,-1, "[WOLRP.NET] {3eedd7}CONG VIEC CUA BAN DA DUOC RESET VE BAN DAU , BAY GIO BAN CO THE LAM VIEC.");
	    SendClientMessage(playerid,-1, "[WOLRP.NET] {eefd03}Neu Ban Khong Biet Lenh Hay Nhan Vao Thanh Cong Cu Nut [ {3eedd7}ALT {eefd03}] {eefd03}Nhe Chuc Anh Em RolePlay Vui Ve ");
	}
	return 1;
}
